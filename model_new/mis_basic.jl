...
data = reduce(vcat, CSV.read.(files, DataFrame); cols=:union)
...
for trial_row in eachrow(data)
    cue_values_str = lpad(string(trial_row.CueValues), 4, '0')   # 4 spatial slots
...
for cc in sort(unique(data.CueCondition))
    ...
    for trial_row in eachrow(cond_rows)
        cue_values_str = lpad(string(trial_row.CueValues), 4, '0')
